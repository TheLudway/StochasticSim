...
<<histograma-fat, echo=TRUE, results='hide', fig=TRUE, dev='pdf', fig.width=6, fig.height=4>>=
# Leer el archivo
...
\begin{table}[ht]
  ...
  \begin{tabular}{|c|c|}
    ...
    $10^{2}$ & $\Sexpr{results[1]}$ \\ \hline
    $10^{3}$ & $\Sexpr{results[2]}$ \\ \hline
    $10^{4}$ & $\Sexpr{results[3]}$\\ \hline
    $10^{5}$ & $\Sexpr{results[4]}$\\ \hline
  \end{tabular}
...
\[
[\Sexpr{results[4, 2]}, \Sexpr{results[4, 3]} ]
\]
